...
ElementCCWVortex = ElementCircle:new();

-- remember this subclass for the editor
ElementBase.elementClasses.ElementCCWVortex = ElementCCWVortex;

function ElementCCWVortex:setDefaults()
  if not self.radius then self.radius = 100 end
  if not self.text then self.text = "CCW vortex" end
  if not self.color then self.color = { r = 35, g = 35, b = 35 } end
  self.class = "ElementCCWVortex"
end

function ElementCCWVortex:modifyBoid(i, boidData, addIfPossible)
	self:setDefaults()
